...
| **HG-1.5** | HG-1 完了直後 | Claude Code で Happy Coder の正常動作を体験する（下記手順） | ユーザーが「正常系の挙動を確認した」と報告 |
| **HG-2** | Task 5 読取 MVP 検証時 | iPhone で Happy アプリを開き、Cursor セッションの表示を目視確認 → 結果を報告 | ユーザーの報告（OK / NG + スクショ） |
| **HG-3** | Task 5 双方向検証時 | iPhone の Happy アプリからメッセージを送信し、Cursor が応答するか確認 → 結果を報告 | ユーザーの報告（OK / NG） |

### HG-1.5: Claude Code で Happy Coder の正常動作を体験

**目的**: Cursor 版の E2E 検証（HG-2, HG-3）で「何が正しい表示か」の基準を持つ。認証フローの動作確認も兼ねる。

**AI が実行:**

```bash
cd /Users/eddy/work/repos/github.com/stlwolf/happy
./packages/happy-cli/bin/happy.mjs claude
```

`happy claude` が起動したら、AI がモバイルからのメッセージ待機状態に入る。

**ユーザーが iPhone で操作:**

1. Happy アプリを開く
2. セッション一覧に新しいセッションが表示されることを確認
3. セッションをタップして開く
4. プロンプトを入力して送信（例: 「このディレクトリのファイル一覧を教えて」）
5. 以下を確認:
   - [ ] テキスト応答がリアルタイムで表示されるか
   - [ ] thinking インジケーター（「考え中...」）が表示されるか
   - [ ] ツール呼び出し（ファイル読み取り、コマンド実行等）が表示されるか
   - [ ] ツール結果が表示されるか
   - [ ] 2回目のメッセージを送信 → 応答が返るか（双方向通信）
   - [ ] abort ボタンで処理を中断できるか

**確認結果の報告**: 上記チェックリストの OK/NG を報告。NG の場合はスクリーンショット添付。

**この体験で得られるもの**:
- iOS アプリの UI・表示パターンの把握（テキスト、thinking、tool_call がどう見えるか）
- Cursor 版（HG-2）で「同じように表示されればOK」の判定基準
- 認証・サーバー接続の正常動作確認
- 公式サーバーへの接続が問題なく行えることの確認

**注意:**
- `happy claude` の実行には Claude Code CLI がインストールされている必要がある
- Claude Code CLI が未インストールの場合、`happy codex` や `happy acp gemini` で代替可能（いずれかの CLI がインストール済みであること）
- この手順で Cursor の API トークンは消費しない（Claude / Codex / Gemini の API を使用）

**注意:**
- HG-1 は Task 1, Task 2, Task 4（ビルド確認）まで到達可能。実装自体はブロックされない
- HG-1.5 は HG-1 の直後に実施。Cursor 版実装の「正解の基準」を作るためのステップ
